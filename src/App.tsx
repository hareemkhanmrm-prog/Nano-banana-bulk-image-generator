...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    for (let i = 0; i < initialResults.length; i++) {
      ...
      try {
        // Perchance unofficial attempt - yeh endpoint change ho sakta hai
        // Real mein Perchance browser se inspect kar ke current form data dekho
        const response = await fetch('https://perchance.org/ai-text-to-image', {
          method: 'POST',
          headers: {
          ...
          body: new URLSearchParams({
            ...
            // optional: add negative prompt etc if needed
        ...
        const data = await response.json(); // assuming JSON with image urls

        // Adjust according to actual response structure (inspect network tab)
        let imageUrl = '';
        if (data.images && data.images.length > 0) {
          imageUrl = data.images[0]; // or data.url or whatever field
        } else if (data.image) {
          imageUrl = data.image;
        }

        if (imageUrl) {
          // agar direct URL nahi to base64 banao ya proxy use karo
          setResults((prev) =>
            prev.map((r) =>
        ...
        setResults((prev) =>
          prev.map((r) =>
            r.id === currentResult.id
              ? { ...r, status: 'error', error: error.message || 'Failed - Perchance may block or changed API' }
              : r
          )
        );
      }
  ...
  const handleDownloadZip = async () => {
    ...
    for (const [index, result] of successfulResults.entries()) {
      try {
        ...
        const safePrompt = result.prompt.replace(/[^a-z0-9]/gi, '_').substring(0, 30);
        zip.file(`${safePrompt}_${index + 1}.png`, blob);
      } catch {}
    }
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        <header className="mb-10 text-center">
          <h1 className="text-4xl font-bold tracking-tight text-neutral-900 mb-3">
            Bulk AI Image Generator (Perchance Free)
          </h1>
          <p className="text-neutral-500 max-w-2xl mx-auto">
            Paste prompts (one per line) → Free unlimited via Perchance (may need tweaks if API changes)
          </p>
        </header>

        {/* Textarea + Generate button same as before */}
